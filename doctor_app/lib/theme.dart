...
ThemeData renyunTheme() {
  final scheme = ColorScheme.fromSeed(seedColor: blue, primary: blue, surface: Colors.white);
  return ThemeData(
    useMaterial3: true,
    colorScheme: scheme,
    ...
    inputDecorationTheme: InputDecorationTheme(
      ...
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: blue, width: 1.6)),
    ),
  );
}
  ...
  Widget build(BuildContext context) {
    final box = Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x120F2744), blurRadius: 24, offset: Offset(0, 10))],
      ),
      child: child,
    );
    ...
    return Material(
      ...
      child: InkWell(onTap: onTap, borderRadius: BorderRadius.circular(22), child: box),
    );
  }
}

Color statusTone(String status) {
  if (status == '标准') return good;
  if (status == '纠正中') return warn;
  return danger;
}
